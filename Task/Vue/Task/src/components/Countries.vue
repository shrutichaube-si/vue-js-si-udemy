<template>
  ...
  export default {
    ...
    filterCountriesByCountryName(e) {
        ...
        const selectedCountry = this.selectedCountries.find(country=>country.id==this.selectedCountry)
      ...
      sendDataToServer(){
      axios.post("http://localhost:8080/addContinents",
      {
        id:this.id,
        name:this.name,
        rank:this.rank,
        selectedContinent:this.selectedContinent}).then((res)=>{
        console.log(res)
