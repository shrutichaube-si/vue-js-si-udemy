...
<div class= "col">
<div class="card" >
<img class="card-img-top" src="{{IMAGE_NAME}}" alt="...">
<div class="card-body">
  <h5 class="card-title">{{PLAYER_NAME}}</h5>
  <p class="card-text">Matches:{{MATCHES}} </p>
  <p class="card-text">Runs:{{RUNS}}</p>
  <p class="card-text">50s/100s:{{50s}}/{{100s}}</p>
  <p class="card-text">Highest_Score:{{HIGHEST_SCORE}}</p>
...
function myfunc(){
  ...
  playersdata.forEach((playerInfo, i) => {

    //for batsman
    if(strr="" || playerInfo.name.toLowerCase().includes(text) ){
...
function onchangeDropdown(value){
  ...
  playersdata.forEach((playerInfo, i) => {
    ...
    //for batsman
    if(playerInfo.team_name == value || value == "ALL"  ){
