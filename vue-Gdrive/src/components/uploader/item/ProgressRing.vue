<template>
    <svg :height="radius * 2" :width="radius * 2">
        <!-- (width/2) - (strikeWidth * 2) -->
        <circle stroke="#ccc" :stroke-width="strokeWidth" fill="transparent" :cx="radius" :cy="radius" :r="normalizedRadius"/>
        <circle class="progress-ring" stroke="#3881ff" :stroke-width="strokeWidth" fill="transparent" :cx="radius" :cy="radius" :r="normalizedRadius" :stroke-dasharray="`${circumference} ${circumference}`" :stroke-dashoffset="strokeDashoffset"/>
    </svg>
</template>
<script>
