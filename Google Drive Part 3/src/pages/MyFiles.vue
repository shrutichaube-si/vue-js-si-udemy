<template>
  <div class="container py-3">
    ...
    <app-modal
      ...
      @hide="modal.rename = false ,newFolder = false"
    >

    <FolderNewForm v-if ="modal.newFolder"  @folder-created ="handleFolderCreated" @close ="modal.newFolder = false"  :folder-id="folderId"/>
      <RenameForm
...
const handleFolderCreated =(folder) =>{
  ...
  const handleDoubleClickFolder  =(folder) => {
    ...
    onMounted  (()=>{
   window.onpopstate = () =>{
      Object.assign(query,fromEntries(new URLSearchParams(window.location.search)));
 }
