  ...
  <header class="site-header">
    <div class="container-fluid clearfix">
      ...
      <div class="pull-right right-wrap">
        <label>Match File Dashboard</label
        ><span>
  ...
  <div class="match-schedule-container">
    <div id="fixturesDiv" class="col-sm-12 col-md-8 col-lg-9 simc-site-leftcol">
      <div class="si-subtab-content simc-international-content">
        ...
        <div class="simc-result-data">
          <div class="simc-table simc-fixture-table">
            <div class="simc-table-tr">
              <div class="simc-table-th simc-tblDate">
                <span>Date &amp; Time</span>
              </div>
              <div class="simc-table-th simc-tblTeams"><span>Teams</span></div>
              <div class="simc-table-th simc-tblMatch"><span>Match</span></div>
              <div class="simc-table-th simc-tblStatus">
                <span>Game Id</span>
              </div>
            </div>

            <div
              v-for="tennis in filteredSeries"
              :key="tennis"
              class="simc-table-tr manipulate matches-list"
            >
              <div class="simc-table-td simc-tblDate">
                <span>{{
                 getDateAndTime(tennis)
                }}</span>
              </div>
              <div class="simc-table-td simc-tblTeams">
                <span
                  >{{ getPlayerName(tennis, "a") }} vs
                  {{ getPlayerName(tennis, "b") }}</span
                >
              </div>
              <div class="simc-table-td simc-tblMatch">
                <span class="simc-txt1"> {{ getMatchType(tennis) }}</span>
              </div>
              <div class="simc-table-td simc-tblStatus match-with-mc">
                <div class="game-id-Wrap">
                  <span class="game-id"
                    >{{ getGameId(tennis,"a") }}:7:{{ getGameId(tennis,"b") }}</span
                  >
                  <button
                    type="button"
                    @click="copyId(tennis.tour_id, 7, tennis.match_id)"
                    class="copy-game-id"
                    style="cursor: pointer"
        ...
        <div class="no-data-avail" style="text-align: center; display: none">
          No Data Available
        </div>
        <div></div>
      </div>
...
export default {
  ...
  methods: {
    getPlayerName(tennis, team) {
      try {
		// console.log(tennis);
        const teamToKeyMapper = {
          a: "teama_info",
          b: "teamb_info",
        };
        const teamKey = teamToKeyMapper[team];
        return tennis[teamKey][0].name;
      } catch (error) {
        console.log(error, "getcountryname error");
        return "-";
      }
    },
	getDateAndTime(tennis){
       try {
		// console.log(tennis)
		let dt = new Date(tennis.match_schedule_date)
		return dt.toDateString()
	   } catch (error) {
		console.log(error,"Date And Time Error")
	   }
	},
	getMatchType(tennis){
		try {
	    let MatchType = (tennis.comp_type);
		return MatchType
		} catch (error) {
			console.error("Match Type Error",error)
		}
	},
	getGameId(tennis,gameid){
		try {
			let GameId = {
				a:"tour_id",
				b:"match_id"
			}
			const gamekey = GameId[gameid];
			return tennis[gamekey]
		} catch (error) {
    ...
    async changeYear() {
      try {
        ...
        if (!tourInfo) {
          console.error(
            `Tour information not found for year ${this.selectedYear}`
          );
          return;
        }
        let res = {
        ...
        const tourDataPromises = tourInfo.tourIds.map(async (tourId) => {
          const tourData = await axios.get(
            config.feedPath.replace("{{TOUR_ID}}", tourId)
          );
    ...
    downloadCsv() {
      try {
        if (!this.Series) {
          ...
          const csvContent =
            "Date & Time,Teams,Match,GameId\n" +
            selectedSeriesData
              .map((row) => {
                const dateAndTime = row.match_schedule_date;
                const teams = `${row.teama_info[0].name} vs ${row.teamb_info[0].name}`;
  ...
  computed: {
    uniqueSeries() {
      ...
      return Array.from(uniqueSeriesMap.values());
			...
			console.error("Unique Series not found",error)
		}
    ...
    filteredSeries() {
        ...
        const FilteredData = this.Tennis.filter(
          (tennis) => tennis.seriesname === this.Series
        );
        return FilteredData;
      }
		} catch (error) {
			console.error("Series Not filtered",error)
		}
    },
	AllSeries(){
		try {
			const AllFilteredData = this.Tennis;
			return AllFilteredData;
			}
		 catch (error) {
			console.error("error in all series",error)
		}
	}
  },
  mounted: async function () {
    try {
      this.selectedYear = config.years.find(
        (yearInfo) => yearInfo.isDefault
      ).year;
