<template>
  <div class="container py-3">
    ...
    >
    <SectionHeader title="Folders" @sort-change="handleSortChange" v-if="folders.length" sort-toggler="true"/>
    <FoldersList
        :folders="folders"
    ...
    <FolderNewForm v-if="modal.newFolder" @folder-created="handleFolderCreated" @close="modal.newFolder = false" :folderId="folderId"/>
      <RenameForm
...
<script>
import filesApi from "../api/folders";
...
export default {
  ...
  setup() {
    ...
    const handleRemove = () => {
      if (confirm("Are you sure?")) {
        selectedItems.value.forEach((item) => {
          if(item.mimeType) {
            removeItem(item,false,filesApi.delete);
          }else{
    ...
    const handleRename = (items,newItem,entity) => {
      ...
      items.value.splice(index, 1, newItem);items
      toast.show = true;
